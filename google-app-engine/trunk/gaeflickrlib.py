...
class GFLPhoto:
    ...
    def __init__(self, photo):
        ...
        for key, value  in photo.attributes.items():
            self.data[key] = value
        for node in photo.getElementsByTagName('*'):
            self.data[node.tagName] = _getNodeText(node.childNodes)
    ...
    def __getitem__(self, key):
        ...
        if key in self.data:
            ...
            raise "No such key '" + str(key) + "' in data: " + str(pprint.PrettyPrinter(indent=4).pformat(self.data))
    
    def __contains__(self,key):
        return key in self.data
    
class GFLUser:
    """Information about a single Flickr user."""
    def __init__(self, rsp):
        self.data = {}
        #logging.info("GFLUser __init__: " + pprint.PrettyPrinter(indent=4).pformat(rsp.__dict__))
        personNode = rsp.getElementsByTagName('person')[0]
        for node in personNode.getElementsByTagName('*'):
            self.data[node.tagName] = _getNodeText(node.childNodes)
        #logging.info("GFLUser: " + pprint.PrettyPrinter(indent=4).pformat(self.data))
...
# people
    ...
    def people_getInfo(self, user_id, **args):
        """Get information for a given user by id"""
        if user_id is None:
            raise GaeFlickrLibException, "user_id not provided"
        else:
            args['user_id'] = user_id
            rsp = self.execute('flickr.people.getInfo', args=args)
            ...
            if user:
                ...
                raise GaeFlickrLibException, "no user"
